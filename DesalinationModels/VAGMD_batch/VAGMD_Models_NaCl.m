function[PFlux,TCO,TEO,PFR,ATml,ThPower,STEC,GOR]=...
    VAGMD_Models_NaCl(TEI_r,FFR_r,TCI_r,Sgl_r,...
...
% Logarithmic mean temperature difference (ATml) [ºC].
ATml = ((TEI_r-TCO)-(TEO-TCI_r)) ./ (log((TEI_r-TCO)./(TEO-TCI_r)));

% Thermal power (ThPower) [kWth].
ThPower = (FFR_r .* CpF .* (TEI_r - TCO)) .* (RhoF / (1000 .* 3600 .* 1000));

% Specific thermal energy consumption (STEC) [kWhth/m3].
