...
class medPsa:
    ...
    Tcwout=Tcwin+7.3    #Tcwout is the seawater outlet temperature of the condenser, ºC
    Tvc=Tcwout+1.5      #Tvc is the vapor temperature inside the condenser, ºC
    Tb=np.zeros((Nef,1))
    Tb[Nef-1]=36 
    Tb[0]=69 
    DELTATb=(Tb[0]-Tb[Nef-1])/(Nef-1) 
    
    for i in range(1,Nef-1):
        Tb[i]=Tb[i-1]-DELTATb 
    ...
    # Calculate the temperature of the brine after flashing process from effects 2 to Nef, Tdb[i]. 
    Tdb=np.zeros((Nef,1))
    for i in range(1,Nef):
        Tdb[i]=Tv[i]-NEA  #ºC 
    ...
    Evh=np.zeros((Nph,1))       # Enthalpy of the vapor that enters the preheater, kJ/kg
    Elh=np.zeros((Nph,1))        # Enthalpy of the condensate that leaves the preheater, kJ/kg
    Lvh=np.zeros((Nph,1))               # Latent heat of the vapor in the preheater, kJ/kg
    Mvh=np.zeros((Nph,1))   # kg/s
    Mdh=np.zeros((Nph,1))                        # Condensate generated in the preheater, kg/s
    Qph=np.zeros((Nph,1))       # Heat transfer rate in the preheaters, kJ/s
    Uph=np.zeros((Nph,1))   # Overall heat transfer coefficient of the preheaters, kW/m2ºC. Correlation of El-Dessouky
    LTMDph=np.zeros((Nph,1))  # Log mean temperature difference, ºC
    Aph=np.zeros((Nph,1)) 
    ...
    for i in range(0,Nph):
        ...
        Mvh[i]=(Mf*Cp*(Tph[i]-Tph[i+1]))/Lvh[i]  # kg/s
        Mdh[i]=Mvh[i]                        # Condensate generated in the preheater, kg/s
        Qph[i]=Mf*Cp*(Tph[i]-Tph[i+1])       # Heat transfer rate in the preheaters, kJ/s
    ...
    Mgf=np.zeros((Nef,1))
    Egbl = np.zeros((Nef,1))    # Enthalpy of the brine that has not evaporated, kJ/kg
    Lgb=np.zeros((Nef,1))         # Latent heat of the generated vapor by boiling of the brine in the first effect, kJ/kg
    Lv=np.zeros((Nef,1))                     # Latent heat of the vapor entering the bundle tube of the second effect (at temperature Tv[0], kJ/kg
    Esg=np.zeros((Nef,1))
    Esl=np.zeros((Nef,1))
    #Ls=np.zeros((Nef,1))
    Mgb=np.zeros((Nef,1))
    Mfv=np.zeros((Nef,1))        # Mass flow rate of the vapor generated in the first effect (by boiling and flashing), kg/h
    Mb=np.zeros((Nef,1))                         # Mass flow rate of the brine that leaves the first effect, kg/s
    Xb=np.zeros((Nef,1))
    Mdb=np.zeros((Nef,1))                                    # Mass flow rate of the brine after flashing process, which is 0 in the first effect, kg/s
    Mdr=np.zeros((Nef,1))                                    # Mass flow rate of the distillate that remains from the distillate branches, which is 0 for the fourth effect, kg/s
    Uef=np.zeros((Nef,1))
    Aef=np.zeros((Nef,1))
    ...
    Esg[0]=F.enthalpySatVapTW(Ts+273.15) 
    Esl[0]=F.enthalpySatLiqTW(Ts+273.15) 
    Ls=Esg[0]-Esl[0] 
    Qs=Ms*Ls 
    Mgb[0]=((Ms*Ls[0])-(Mf*Cp*(Tb[0]-Tf)))/Lgb[0] 
    Mfv[0]=(Mgb[0]+Mgf[0])*3600               # Mass flow rate of the vapor generated in the first effect (by boiling and flashing), kg/h
    Mb[0]=Mf-Mgb[0]                             # Mass flow rate of the brine that leaves the first effect, kg/s
    Xb[0]=(Mf*Xf)/Mb[0] 
    ...
    Aef[0]=Qs/(Uef[0]*(Ts-Tb[0])) 
    
    # Calculations in the effects from 2 to Nef
    ...
    for i in range(1,Nef):
              ...
              Lgf[i]=Lv[i]  
              Lgb[i]=Lv[i]  
              Mgf[i]=(Mb[i-1]*Cp*(Tbn[i-1]-Tdb[i]))/Lgf[i]  
              Mdb[i]=Mb[i-1]-Mgf[i] 
              ...
              if(i==1)or(i==4)or(i==7)or(i==10)or(i==13):    
                     ...
                     Tdd[i]=Td[i]-2 
                     Mgb[i]=((Mv[i]*Lv[i-1])+(Mdb[i]*Cp*(Tdb[i]-Tbn[i]))+(Mdh[i-1]*Cp*(Tv[i-1]-Tdv[i-1])))/Lgb[i]  
                     ...
                     Tdd[i]=Td[i]-2 
                     Mgb[i]=((Mv[i]*Lv[i-1])+(Mdb[i]*Cp*(Tdb[i]-Tbn[i]))+(Mdh[i-1]*Cp*(Tv[i-1]-Tdv[i-1]))+(Md[i-1]*Cp*(Td[i-1]-Tdd[i-1])))/Lgb[i]  # Mass flow rate of the vapor generated by boiling of the brine, kg/s   
                     Mfv[i]=(Mgb[i]+Mgf[i])*3600 
                     Qef[i]=Mv[i]*Lv[i-1] 
                     ...
                     if (i==6):
                            ...
                            Tddm[i]=Tdm[i]-6    
                            Mgb[i]=((Mv[i]*Lv[i-1])+(Mdb[i]*Cp*(Tdb[i]-Tbn[i]))+(Mdh[i-1]*Cp*(Tv[i-1]-Tdv[i-1]))+(Md[i-1]*Cp*(Td[i-1]-Tdd[i-1]))+(Mda[i]*Cp*(Td[i-3]-Tda[i])))/Lgb[i] 
                            Mfv[i]=(Mgb[i]+Mgf[i])*3600 
                            Qef[i]=Mv[i]*Lv[i-1] 
                     elif (i==9)or(i==12):
                           ...
                           Tddm[i]=Tdm[i]-6 
                           Mgb[i]=((Mv[i]*Lv[i-1])+(Mdb[i]*Cp*(Tdb[i]-Tbn[i]))+(Mdh[i-1]*Cp*(Tv[i-1]-Tdv[i-1]))+(Md[i-1]*Cp*(Td[i-1]-Tdd[i-1]))+(Mda[i]*Cp*(Tdm[i-3]-Tddm[i-3])))/Lgb[i] 
                           Mfv[i]=(Mgb[i]+Mgf[i])*3600 
                           Qef[i]=Mv[i]*Lv[i-1] 
    ...
    Mdbc=Mdr[Nef-2]+Md[Nef-2]+Md[Nef-1]  #kg/s
    Tdbc=((Mdm[Nef-2]*Td[Nef-2])+(Md[Nef-1]*Td[Nef-1]))/Mdbc  #ºC
    ...
    Mdf=(Mdbc*Cp*(Tdbc-Tvc))/Ldf  # Mass flow rate of the vapor generated by flashing of the condensate entering the condenser, kg/s
    #Lv=Ldf 
    Msw=((Mgb[Nef-1]+Mgf[Nef-1]+Mdf)*(1-Mventc)*Ldf)/(Cp*(Tcwout-Tcwin))  # Mass flow rate of the cooling seawater in the condenser, kg/s
    Uc=1.7194+(3.2063e-3*Tv[Nef-1])+(1.5971e-5*Tv[Nef-1]**2)-(1.9918e-7*Tv[Nef-1]**3)  
    Qc=Msw*Cp*(Tcwout-Tcwin) 
    LTMDc=((Tv[Nef-1]-Tcwin)-(Tv[Nef-1]-Tcwout))/math.log((Tv[Nef-1]-Tcwin)/(Tv[Nef-1]-Tcwout)) 
    Ac=Qc/(Uc*LTMDc) 
    
    # Calculation of the final product mass flow rate, kg/s
    Mpro=Mgb[Nef-1]+Mgf[Nef-1]+Mdbc 
    Tpro=((Mgb[Nef-1]*Egv[Nef-1])+(Mgf[Nef-1]*Egv[Nef-1])+(Mdf*Edfg))/(Mpro*Cp) 
    Mprod_final=Mpro*24*3.6 
    ...
    sA=(Sum_Aef+Sum_Aph+Ac)/Mprod_final 
